...
{
	// ピクセルシェーダーの作成
	std::vector<uint8_t> ps_fade = DX::ReadData(L"Resources/Shaders/Fade_PS.cso");
	DX::ThrowIfFailed(
	...
	// マスク用テクスチャの読み込み
	DX::ThrowIfFailed(
		CreateDDSTextureFromFile(device, L"Resources/dds/FadeMask.dds", nullptr, m_maskTexture.ReleaseAndGetAddressOf())
	);
}
...
)
{
	...
	m_graphics->GetSpriteBatch()->Begin(SpriteSortMode_Immediate, states->NonPremultiplied(), nullptr, nullptr, nullptr, [&]()
		{
			// 定数バッファの設定
	...
	m_graphics->GetSpriteBatch()->Draw(texture, rect);
